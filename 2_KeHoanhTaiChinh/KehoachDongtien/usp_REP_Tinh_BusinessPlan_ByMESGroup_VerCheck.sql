...
BEGIN
    SET NOCOUNT ON;
    -- ======================================================== -- 1. KH?I T?O BI?N VÀ B?NG T?M C? S? -- ========================================================
    ...
    SELECT @_Step = N' Start: Kh?i t?o d? li?u c? b?n & GenKey xong.';
    ...
    -- V?LA s?a riêng cho THACO -- X? lý d? li?u ??n v? c? s? và n?m làm vi?c
    ...
    SET ProductLineId = NULL
      ...
      , ProductClassId = NULL
      , ProductLevelId = NULL;
    SELECT @_Step = N' B??c 1: L?y xong d? li?u S? phát sinh s? cái (#CtTmpGeneralLedger).';
    EXECUTE dbo.usp_sys_CheckDebugTime @_StepName = @_Step
                                     , @_DebugStepTime = @DebugStepTime OUTPUT;
    ...
    -- ========================================================    
    -- 4.1 B? sung thêm ?o?n update riêng cho các ??n v? theo dõi doanh thu theo H?p ??ng (I24)
    -- ========================================================	
...
FROM #CtTmpGeneralLedger kq INNER JOIN dbo.B3' + @_DataCode_Filter
      ...
      IF EXISTS
        ...
        END;
    END;
    SELECT @_Step = N' B??c 4. Xong Update các tr??ng Code,Name theo danh m?c cho b?ng #CtTmpGeneralLedger';
    ...
    EXECUTE dbo.usp_sys_Append @_TableSource = @_DefinitionTableName
                             ...
                             , @_Where_TableSource = @_Key; -- Thêm các c?t ??ng vào khung báo cáo
    ...
    BEGIN
        ...
        BEGIN
            ...
            INTO #kq_GV
            FROM #CtTmpGeneralLedger;
            SELECT @query_tmp = @query + N'  AND   Account LIKE ''632%'' AND CrspAccount NOT LIKE ''911%''  ';
    ...
    BEGIN
        SELECT @_Message = N'Các ??n v? c?n l?u ý xem l?i !' + NCHAR(13) + NCHAR(10) + @_BranchLst_Err;
    END;
    UPDATE #tblTmp1Grid2_Result
    ...
    SET @_DebugMsg
        = N'--- T?NG TH?I GIAN CH?Y SP: ' + CAST(DATEDIFF(ms, @_Time1, GETDATE()) AS VARCHAR) + N' ms ~ '
...
--set @p17='00:00:01'
DECLARE @p19 VARCHAR(MAX);
--set @p19=''
EXEC usp_REP_Tinh_BusinessPlan_ByMESGroup_VerCheck @_DocDate1 = '2026-01-01'
                                                      , @_DocDate2 = '2026-06-30'
                                                      , @_DocDatePlan2 = DEFAULT
                                                      , @_ProductId = DEFAULT
                                                      , @_BranchCode = 'I17'
                                                      , @_Account = DEFAULT
                                                      , @_ExcludeCrspAccount = DEFAULT
                                                      , @_nUserId = 1213
                                                      , @_LangId = 0
                                                      , @_CurrencyCode0 = 'VND'
                                                      , @_LAYOUT_XML = @p11 OUTPUT
                                                      , @_IsRound = 0
                                                      , @_DefinitionTableName = DEFAULT
                                                      , @_tblTmp = DEFAULT
                                                      , @_RepId1 = 'I000000007'
                                                      , @_BranchReportId = DEFAULT
                                                      , @_StrTime = @p17 OUTPUT
                                                      , @_IsGetPlan = 0
                                                      --, @_Message = @p19output
                                                      , @_M_BranchFollowRevenueByBizdoc = DEFAULT
                                                      , @_Is_Diff_Branch = 0;
