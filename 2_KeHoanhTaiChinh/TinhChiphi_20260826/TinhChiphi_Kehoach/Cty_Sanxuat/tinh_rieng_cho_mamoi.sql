...
BEGIN
    ...
    --            + ' ---  Done: usp_B30FinPlanDetail_CalculateAndSavePlannedCOGS!!!'
    -- END;
    UPDATE #tbl
    SET _Check = 0
    ...
    BEGIN
        ...
        DELETE FROM #tbl
        WHERE DocDate = @_DocDate;
        EXEC dbo.usp_B30FinPlanDetail_CalculateAndSavePlannedCOGS_Product_New @_BranchCode = @_BranchCode
                                                                            , @_DocDate = @_DocDate
                                                                            , @_Year = @_Year
                                                                            , @_nUserId = 1213
                                                                            , @_SelectExec = 0;
        SELECT @_BranchCode + '---' + FORMAT(@_DocDate, 'dd-MM-yyyy') + '--- Done: usp_B30FinPlanDetail_CalculateAndSavePlannedCOGS_Product_New!!!'
    END;
