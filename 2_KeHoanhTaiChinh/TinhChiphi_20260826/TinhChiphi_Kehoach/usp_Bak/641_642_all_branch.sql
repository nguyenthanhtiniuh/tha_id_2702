...
BEGIN
    ...
    WHILE EXISTS (SELECT 1 FROM #tbl WHERE _Check = 0)
    BEGIN
        SELECT TOP 1
               @_DocDate = DocDate
        FROM #tbl
        ...
        EXEC dbo.usp_B30PlannedSellExpenses_CalculateAndSavePlanned @_BranchCode = @_BranchCode
                                                                  , @_DocDate = @_DocDate
                                                                  , @_Year = @_Year
                                                                  , @_nUserId = 1213
                                                                  , @_SelectExec = 0  , @_Account = '641'


        SELECT @_BranchCode + ' --- ' + FORMAT(@_DocDate, 'dd-MM-yyyy')
               + ' ---  Done: usp_B30PlannedSellExpenses_CalculateAndSavePlanned!!!'
        EXEC usp_B30PlannedAdExpenses_CalculateAndSavePlanned @_BranchCode = @_BranchCode
                                                            , @_DocDate = @_DocDate
                                                            , @_Year = @_Year
                                                            , @_nUserId = 1213
                                                            , @_SelectExec = 0  , @_Account = '642'

        SELECT @_BranchCode + ' --- ' + FORMAT(@_DocDate, 'dd-MM-yyyy')
               + ' ---  Done: usp_B30PlannedAdExpenses_CalculateAndSavePlanned!!!'
