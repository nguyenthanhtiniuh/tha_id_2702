...
*/
CREATE OR ALTER PROCEDURE dbo.usp_B30PlannedSellExpenses_CalculateAndSavePlanned
...
BEGIN
    ...
    EXEC usp_KTH10_v2 @_DocDate = @_DocDate
                    ...
                    , @_tblOutput = '#PlannedSellExpensesResult'
                    , @_ExpenseAccount = @_Account

    EXEC dbo.usp_sys_CreateTable @_Table = '#PlannedSellExpensesResult'
                               , @_BaseTable = 'B30PlannedCostResult'

    UPDATE #PlannedSellExpensesResult
    ...
    CREATE TABLE #PlannedSellExpenses
    (
    ...
    INSERT INTO #PlannedSellExpenses
    (
        BranchCode
      , FiscalYear
      , DocDate
    ...
    SELECT BranchCode
         , FiscalYear
         , DocDate
         , 'KHCPBH' + FORMAT(@_DocDate, 'dd-MM-yyyy') + '' AS DocNo
         , Account
    ...
    DECLARE @_TableDestination       NVARCHAR(256) = N'B30PlannedSellExpenses'
          , @_TableDestinationResult NVARCHAR(256) = N'B30PlannedSellExpensesResult'
    ...
    SELECT @_strExec
        = N' DELETE ' + @_TableDestination + N' WHERE DocDate = ''' + CAST(@_SOMONTH AS VARCHAR(11))
          + N'''  AND  Account IN ( SELECT Account FROM #PlannedSellExpenses  )    '
    EXEC (@_strExec)
    EXECUTE dbo.usp_sys_Append @_TableSource = N'#PlannedSellExpenses'
                             , @_TableDestination = @_TableDestination


    SELECT @_strExec
        = N' DELETE ' + @_TableDestinationResult + N' WHERE DocDate = ''' + CAST(@_SOMONTH AS VARCHAR(11))
          + N'''  AND Account IN ( SELECT Account FROM #PlannedSellExpensesResult  )'
    EXEC (@_strExec)
    EXECUTE dbo.usp_sys_Append @_TableSource = N'#PlannedSellExpensesResult'
    ...
    _End:
    DROP TABLE IF EXISTS #PlannedSellExpenses
                       , #PlannedSellExpensesResult
END
GO
...
SET DATEFORMAT DMY
EXEC usp_B30PlannedSellExpenses_CalculateAndSavePlanned @_DocDate = '25/08/2026 00:00:00.000'
                                                      , @_Year = '2026'
                                                      , @_Account = '641'
                                                      , @_nUserId = 1213
                                                      , @_BranchCode = 'I09',@_SelectExec = 1
