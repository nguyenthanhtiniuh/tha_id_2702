...
BEGIN
    ...
    BEGIN
        ...
        EXEC usp_B30PlannedFinExpenses_CalculateAndSavePlanned @_BranchCode = @_BranchCode
                                                             , @_DocDate = @_DocDate
                                                             , @_Year = @_Year
                                                             , @_nUserId = 1213
                                                             , @_SelectExec = 0
        ...
        SELECT @_BranchCode + ' --- ' + FORMAT(@_DocDate, 'dd-MM-yyyy')
               + ' ---  Done: usp_B30PlannedFinExpenses_CalculateAndSavePlanned!!!'
        ...
        SET DATEFORMAT DMY
        EXEC usp_Tth_AssetSummaryTableByTime_KHTC @_BranchCode = @_BranchCode
                                                , @_DocDate = @_DocDate
                                                , @_Year = @_Year
                                                , @_nUserId = 1213
                                                , @_SelectExec = 0
                                                , @_Account = '214'
        SELECT @_BranchCode + ' --- ' + FORMAT(@_DocDate, 'dd-MM-yyyy')
               + ' ---  Done: usp_Tth_AssetSummaryTableByTime_KHTC!!!'
