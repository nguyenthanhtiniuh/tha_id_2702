...
-- =============================================
ALTER  PROCEDURE [dbo].[usp_B30AssetDoc_CalDepreciation1] 
...
BEGIN
	...
	'					ProductionCapacity,AllocateByCapacity,LastSuspendDate,LastResumeDate)'+@_nl+
	...
	'			ResumeDate.LastTransDate AS LastResumeDate'+@_nl+
 ...
 ELSE
	...
	END
	 ...
	 SET @_cmd = N' UPDATE #t_BienDongCuoi '+@_nl+
				 N'	SET ChkLastDayFluctuations = 1 '+@_nl+
				 N'	FROM #t_BienDongCuoi bd '+@_nl+
				 N'	INNER JOIN B3' + @_DataCode_Branch + N'AssetDoc ts ON bd.AssetId = ts.AssetId '+@_nl+ 
				 N'		AND bd.EquityId = ts.EquityId AND ts.IsActive = 1 '+@_nl+
				 N'		AND ts.DocDate BETWEEN bd.LastDayFluctuations '+@_nl+
				 N'		--AND DATEADD(day, -1, DATEADD(m, 1, CONVERT(DATE, ''01/'' + STR(MONTH(bd.LastDayFluctuations), 2) + ''/'' + STR(YEAR(bd.LastDayFluctuations), 4), 103))) '+@_nl+
				 N'		AND DATEADD(DAY,-1,DATEADD(m,1,FORMAT(bd.LastDayFluctuations,''yyyyMM01''))) '+@_nl+
				 N'		AND ts.AssetTransType = ''KHAUHAO'' '+@_nl+
				 N'		AND ISNULL(IsDeprAdjusted, 0) = 0 '+@_nl+
	...
	CREATE TABLE #t_GtCl0 
		(AssetId INT,AssetCode VARCHAR(24), EquityId INT, FirstDeprDate DATE, UsefulMonth NUMERIC(18, 9),
		 ...
		 _IncOriginalCost NUMERIC(18,2),_DeOriginalCost NUMERIC(18,2),
		 _IncDepreciation NUMERIC(18,2),_DeDepreciation NUMERIC(18,2)
		 )
	...
	SET @_cmd = N' INSERT #t_GtCl0(AssetId,AssetCode,EquityId,FirstDeprDate,UsefulMonth,ProductionCapacity,	 '+@_nl+
				...
				'					_IncOriginalCost,_DeOriginalCost,_IncDepreciation,_DeDepreciation)  '+@_nl+
				...
				N'			SUM(Dk.IncOriginalCost - Dk.DeOriginalCost) - '+@_nl+
				N'				IIF(MAX(BienDongCuoi.LastDayFluctuations) = MAX(dmts.FirstDeprDate), 0, SUM(Dk.IncDepreciation - Dk.DeDepreciation)) AS NetBookValue, '+@_nl+
				...
				N'          SUM(Dk.IncOriginalCost) AS _IncOriginalCost,SUM(DeOriginalCost) AS _DeOriginalCost,'+@_nl+
				'			SUM(Dk.IncDepreciation) AS _IncDepreciation,SUM(DeDepreciation) AS  _DeDepreciation'+@_nl+
	...
	--IF HOST_NAME()LIKE '%TRUNGHT%'
	--		BEGIN
	--			SELECT _IncDepreciation,_DeDepreciation,* FROM #t_GtCl0
	--			SELECT * FROM #t_BienDongCuoi
	--			SELECT SUM(IncDepreciation),SUM(DeDepreciation) 
	--			FROM vB32011AssetDoc WHERE AssetId=3383828 AND DocDate<=@_DocDate1 AND DocDate<='2025-11-01'
	--					AND IsActive=1
 ...
 -- Tính phân bổ CCDC tròn tháng
	...
	--select MonthForDepr1 from  #t_GtCl0 return 
  
  --select FirstDeprDate,@_DocDate1,MonthForDepr1 from #t_GtCl0 return 
  --select MonthForDepr1,ResidualMonth,MonthForDepr1,Depreciation0,* from #t_GtCl0 return 
		...
		CREATE TABLE #_CtCl(AssetId INT,EquityId INT,DeprMethod TINYINT)

		SET @_Cmd = N'INSERT INTO #_CtCl(AssetId,EquityId,DeprMethod) '+@_nl+
					N'	SELECT AssetId, EquityId,DeprMethod '+@_nl+
					...
					N'		GROUP BY AssetId, EquityId,DeprMethod'  
		...
		-- Với CCDC nào không có biến động thì tính Phân bổ tháng = NetBookValue / Số tháng
			...
			SET @_Cmd = N'UPDATE #t_GtCl0 
				...
				FROM #t_GtCl0 a INNER JOIN #_CtCl b ON a.AssetId = b.AssetId AND a.EquityId = b.EquityId 
 ...
 -- Gộp 2 loại để ra phần tính
		...
		UPDATE #t_khts SET LastDeprDate = DATEADD(day, -DAY(DecreaseDate), DATEADD(m, 1, DecreaseDate))
		WHERE DecreaseDate IS NOT NULL AND DecreaseDate BETWEEN @_DocDate1 AND @_DocDate2
 ...
 -- Nếu sản lượng trong tháng + tổng sản lượng đã phân bổ >= tổng sản lượng dự kiến thì mặc định bằng Giá trị còn lại
	...
	SET @_cmd=';WITH Cte '+@_nl+
		...
		'       FROM Cte GROUP BY AssetId) b ON a.Id=b.AssetId ' 
	...
	--IF @_AssetType IN (0, 1 )
	--AND EXISTS (SELECT * FROM #t_khts 
	--WHERE MonthDepr <> (NetBookValue / SoThangConLai)
	--AND SoThangConLai <> 0
	--)
	--BEGIN
	---- 26-02-2026 Với CCDC, tính lại khấu hao tháng dựa trên giá trị còn lại và số tháng còn lại để tránh trường hợp khấu hao âm khi có biến động tăng giảm trong kỳ
	--	UPDATE #t_khts SET MonthDepr = NetBookValue / SoThangConLai
	--	WHERE AllocateByCapacity  <> 1
	--	AND SoThangConLai <> 0 
	--END 
	...
	BEGIN  
		UPDATE #t_khts 
		SET IsDeprAdjusted = 1, MonthDepr = ctts.IncDepreciation
		FROM #t_khts khts 
		INNER JOIN (
...
-- Thanh lý ngay trong tháng đầu tiên tăng
	SET @_cmd = N';WITH TLPB AS (  '+@_nl+
				...
				N' ISNULL(pb.DeprAllocationRate, 0) AS DeprAllocationRate,SoThangDaKhauHao,SoThangConLai  '+@_nl+
