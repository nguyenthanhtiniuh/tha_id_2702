...
BEGIN
    ...
    CREATE TABLE #temptable_BP
        ...
        ,ItemId         INT
        ,ItemCode       VARCHAR(24)
        ,ItemName       NVARCHAR(256) DEFAULT ''
        ,CostFactorId   INT
        ,CostFactorCode VARCHAR(24)
        ,CostFactorName NVARCHAR(256) DEFAULT ''
        ,Quantity       DECIMAL(18, 2)
        ,Coeff          DECIMAL(38, 4)
        ,VariableAmount DECIMAL(38, 6)
        ,ProductId      INT
        ,ProductCode    VARCHAR(24)
        ,ProductName    NVARCHAR(256) DEFAULT ''
    ...
    CREATE TABLE #PlannedCostResult
        ...
        ,Coefficient0   NUMERIC(28, 10) NULL DEFAULT 0
        ...
        ,FixedAmount    NUMERIC(28, 10) NULL DEFAULT 0)
    ...
    EXEC dbo.usp_sys_CreateTable @_Table = '#PlannedCostResult', @_BaseTable = 'B30PlannedCostResult'

    EXECUTE dbo.usp_sys_Append @_TableSource = N'#temptable_BP', @_TableDestination = N'#PlannedCostResult'

    EXECUTE dbo.usp_sys_Append @_TableSource = N'#temptable_DP', @_TableDestination = N'#PlannedCostResult'
    ...
    EXEC (@_strExec)
    EXECUTE dbo.usp_sys_Append @_TableSource = N'#PlannedCost', @_TableDestination = @_TableDestination
    ...
    SELECT @_strExec
        = N' DELETE ' + @_TableDestinationResult + N' WHERE DocDate = ''' + CAST(@_SOMONTH AS VARCHAR(11))
          + N'''  AND  ItemId IN ( SELECT ItemId FROM #PlannedCostResult  )    '
    EXEC (@_strExec)
    EXECUTE dbo.usp_sys_Append @_TableSource = N'#PlannedCostResult', @_TableDestination = @_TableDestinationResult

	--18-07-2026 TINNT
	--b? sung thêm hàm join l?y d? li?u tr??ng "Name" các danh m?c Item,Product,CostFactor t? b?ng danh m?c, l?y t? d? li?u XML s? b? l?i n?u chu?i XML ph?n tag tr??ng "Name" có d?u cách
	EXEC dbo.usp_sys_Update_Col_By_List_Cats @_CtTmp = '#PlannedCostResult', @_List_Cats = 'Item'
	EXEC dbo.usp_sys_Update_Col_By_List_Cats @_CtTmp = '#PlannedCostResult', @_List_Cats = 'Product'
	EXEC dbo.usp_sys_Update_Col_By_List_Cats @_CtTmp = '#PlannedCostResult', @_List_Cats = 'CostFactor'
 
    --18-07-2026 TINNT
    --Tái s? d?ng tr??ng d? li?u Code,Name ?ã get t? b?ng ???c kh?i t?o t? d? li?u @_DataXML. H?n ch? truy xu?t t?i b?ng B20 danh m?c ?? t?ng t?c th?i gian ch?y tính n?ng
    SELECT @_strExec
        = N' ;WITH	CTE_Item	AS (SELECT DISTINCT pcr.ItemId,pcr.ItemCode,pcr.ItemName FROM #PlannedCostResult AS pcr),
          ...
          + N'''  AND  PC.ItemId IN ( SELECT ItemId FROM #PlannedCostResult  )  ORDER BY pcrItem.ItemCode ASC  ' EXEC (@_strExec)   
      ...
      SELECT @_strExec
        = N' ;WITH	CTE_Item	AS (SELECT DISTINCT pcr.ItemId,pcr.ItemCode,pcr.ItemName FROM #PlannedCostResult AS pcr),
				...
				SELECT PCR.*,pcrItem.ItemCode,pcrItem.ItemName
									...
									,CreatedBy.FullName AS CreatedName
									,ModifiedBy.FullName AS ModifiedName
					FROM  ' + @_TableDestinationResult+ N' PCR 
						...
						LEFT JOIN CTE_CostFactor pcrCostFactor ON PCR.CostFactorId  = pcrCostFactor.CostFactorId 
						 ...
						 LEFT JOIN dbo.B00UserList AS ModifiedBy WITH (NOLOCK) ON PCR.ModifiedBy = ModifiedBy.Id
							WHERE DocDate = ''' + CAST(@_SOMONTH AS VARCHAR(11))
          + N'''  AND  PCR.ItemId IN ( SELECT ItemId FROM #PlannedCostResult  )  ORDER BY pcrItem.ItemCode ASC,pcrCostFactor.CostFactorCode ' EXEC (@_strExec)
