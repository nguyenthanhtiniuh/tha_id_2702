...
-- Coder: DucCM    
-- Manager: ThangNH
ALTER   PROCEDURE dbo.usp_Tth_ToolInstrumentAllocationCalc_test 
...
BEGIN    
	...
	--L?y t?t c? d? li?u Kh?u hao CCDC t?i ngày DocDate2
    ...
    SELECT tb.AssetId    
          ...
          ,SUM(CASE WHEN tb.DocDate < @_DocDate1  
                    ...
                    ELSE CAST(0 AS NUMERIC(18,2)) END) AS OpenOriginalCost  --Giá tr? ??u k?  
          ,SUM(CASE WHEN tb.DocDate < @_DocDate1  
                    THEN IIF(tb.AssetTransType = 'GIAMTAISAN', 0, tb.Depreciation * tb._Head)  
                    ELSE CAST(0 AS NUMERIC(18,2)) END) AS OpenDepreciation  --Giá tr? phân b? ??u k?
          ,CAST(0 AS NUMERIC(18,2))   AS OpenBookValue						--Giá tr? còn l?i ??u k?
          ...
          ,SUM(CASE WHEN tb.DocDate BETWEEN @_DocDate1 AND @_DocDate2  
                    ...
                    ELSE CAST(0 AS NUMERIC(18,2)) END) AS ThisPeriodDepreciation    --Giá tr? phân b? trong k?

          ,SUM(IIF(DocDate BETWEEN @_DocDate1 AND @_DocDate2, IncOriginalCost,  0)) AS ThisPeriodIncOriginalCost    --Giá tr? t?ng trong k?
          ,SUM(IIF(DocDate BETWEEN @_DocDate1 AND @_DocDate2, DeOriginalCost,   0)) AS ThisPeriodDeOriginalCost		--Giá tr? gi?m trong k?   
          
		  --Cu?i k?
		  ,SUM(tb.OriginalCost * tb._Head) AS CloseOriginalCost    --Giá tr? cu?i k?
          ,SUM(IIF(tb.AssetTransType = 'GIAMTAISAN', tb.NetBookValue, tb.Depreciation * tb._Head)) AS CloseDepreciation    
		  --Giá tr? phân b? l?y k? cu?i k?
          ,CAST(0 AS NUMERIC(18,2))   AS CloseBookValue 
		  --Giá tr? cu?i k?
...
/*
OpenOriginalCost	Giá tr? ??u k?
OpenDepreciation	Giá tr? phân b?
OpenBookValue	Giá tr? còn l?i
ThisPeriodIncOriginalCost	T?ng
ThisPeriodDeOriginalCost	Gi?m
ThisPeriodDepreciation	Giá tr? phân b? trong k?
CloseOriginalCost	Giá tr? cu?i k?
CloseDepreciation	Giá tr? phân b?
CloseBookValue	Giá tr? còn l?i
*/	

	
	--l?y d? li?u gi?m CCDC, d? li?u thanh lý CCDC
    UPDATE #BcTemp    
    ...
    FROM #BcTemp bct    
		...
		DECLARE @_ListCol_Update NVARCHAR(MAX) 
		
		
  
    ;WITH cte AS (SELECT AssetId, ExpenseCatgId, DocDate FROM #CtTmp WHERE AssetTransType <> 'GIAMTAISAN')  
    UPDATE #BcTemp SET ExpenseCatgId = a.ExpenseCatgId  
    FROM #BcTemp bct OUTER APPLY (SELECT TOP 1 * FROM cte WHERE bct.AssetId = cte.AssetId ORDER BY cte.DocDate DESC) a  
  
    ;WITH cte AS (SELECT AssetId, DeptId, DocDate FROM #CtTmp WHERE AssetTransType <> 'GIAMTAISAN')  
    UPDATE #BcTemp SET DeptId = a.DeptId  
    FROM #BcTemp bct OUTER APPLY (SELECT TOP 1 * FROM cte WHERE bct.AssetId = cte.AssetId ORDER BY cte.DocDate DESC) a  
 
    ;WITH cte AS (SELECT AssetId, StageId, DocDate FROM #CtTmp WHERE AssetTransType <> 'GIAMTAISAN')  
    UPDATE #BcTemp SET StageId = a.StageId  
    FROM #BcTemp bct OUTER APPLY ( SELECT TOP 1 * FROM cte WHERE bct.AssetId = cte.AssetId ORDER BY cte.DocDate DESC) a  
    ...
    ;WITH cte AS (SELECT AssetId, ProfitCenterId, DocDate FROM #CtTmp WHERE AssetTransType <> 'GIAMTAISAN')  
    UPDATE #BcTemp SET ProfitCenterId = a.ProfitCenterId  
    FROM #BcTemp bct OUTER APPLY (SELECT TOP 1 * FROM cte WHERE bct.AssetId = cte.AssetId ORDER BY cte.DocDate DESC) a  
 
    ;WITH cte AS (SELECT AssetId, DeprDebitAccount, DocDate FROM #CtTmp WHERE AssetTransType <> 'GIAMTAISAN')  
    UPDATE #BcTemp SET DeprDebitAccount = a.DeprDebitAccount  
    FROM #BcTemp bct OUTER APPLY (SELECT TOP 1 * FROM cte WHERE bct.AssetId = cte.AssetId ORDER BY cte.DocDate DESC    ) a  
 
    ;WITH cte AS (SELECT AssetId, DeprCreditAccount, DocDate FROM #CtTmp WHERE AssetTransType <> 'GIAMTAISAN')  
    UPDATE #BcTemp SET DeprCreditAccount = a.DeprCreditAccount  
    FROM #BcTemp bct OUTER APPLY (SELECT TOP 1 * FROM cte WHERE bct.AssetId = cte.AssetId ORDER BY cte.DocDate DESC) a  
    ...
    -- L?y d? li?u danh sách s?n ph?m ?ã kh?u hao theo s?n l??ng  
    ...
    FROM #BcTemp ct    
        LEFT OUTER JOIN dbo.B20ExpenseCatg expe      ON expe.Id      = ct.ExpenseCatgId    
        LEFT OUTER JOIN dbo.B20ProfitCenter pro       ON pro.Id       = ct.ProfitCenterId    
        LEFT OUTER JOIN dbo.B20Dept         dept      ON dept.Id      = ct.DeptId    
        LEFT OUTER JOIN dbo.B20ZStage       Stage     ON Stage.Id     = ct.StageId    
        LEFT OUTER JOIN dbo.B20Territory        Territory ON Territory.Id = ct.TerritoryId    
...
exec usp_Tth_ToolInstrumentAllocationCalc_test @_DocDate1='2026-07-01',@_DocDate2='2026-07-31',@_AssetId=default,@_lookupModeAsset=default,@_DeptId=default,@_AssetAccount=default,@_DeprDebitAccount=default,@_DeprCreditAccount=default,@_nUserId=1213,@_LangId=0,@_CurrencyCode0='VND',@_BranchCode='I01',@_DataCode='2001',@_IsHide=0,@_LAYOUT_XML=@p15 output,@_CheckBC=1,@_DeprMethod=default,@_GroupByExprs=default,@_TableTemp=default,@_BranchReportId=default
